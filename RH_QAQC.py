...
for l in range(len(wx_stations_name)):
    ...
    # off clean db that goes beyond extent of qaqc db
    qaqc_upToDate_clean = (datetime.now()- dtime.timedelta(days=7)).strftime("%Y-%m-%d %H") + ':00:00' # todays date rounded to nearest hour
    sql_file_idx_latest_clean = int(np.flatnonzero(sql_file['DateTime'] == qaqc_upToDate_clean)[0]) if np.flatnonzero(sql_file['DateTime'] == qaqc_upToDate_clean).size > 0 else 0   # today's date - 7 days  
    qaqc_file_airTemp = pd.read_sql(sql="SELECT * FROM qaqc_" + sql_database, con = engine) # get air temp from already qaqced data
    stop_sql_idx = int(np.flatnonzero(sql_file['DateTime'] == str(qaqc_file_airTemp['DateTime'].iloc[-1]))[0]) if np.flatnonzero(sql_file['DateTime'] == str(qaqc_file_airTemp['DateTime'].iloc[-1])).size > 0 else 0   # today's date - 7 days
    sql_file = sql_file[:stop_sql_idx+1]
    ...
    if sql_file_idx_latest_clean != 0:
        ...
        for k in range(len(yr_range)-1,len(yr_range)):
            ...
            #%% Remove values above 100 or below 0.5% threshold
            # above 100%
            ...
            #%% Remove duplicate consecutive values == 100% or == 0.5% for RH
            # equal to 100% for specific window size
            data = qaqc_arr[var].iloc[np.arange(dt_yr[0].item(),dt_yr[1].item()+1)]
            flag = 3
            window = 120 # in hours (equivalent to 5 days)
            threshold = 100
            qaqc_3, flags_3a = qaqc_functions.duplicates_window(qaqc_arr[var], data, flag, window, threshold)
            qaqc_arr[var] = qaqc_3
            ...
            window = 12 # in hours
            threshold = 0.5
            qaqc_3, flags_3b = qaqc_functions.duplicates_window(qaqc_arr[var], data, flag, window, threshold)
            qaqc_arr[var] = qaqc_3
            
            # merge flags
            flags3 = pd.concat([flags_3a,flags_3b],axis=1) # merge so you don't lose previous flags 3
            flags_3 = flags3.apply(qaqc_functions.merge_row, axis=1).astype(int) # merge so you don't lose previous flags 3
            ...
            data_temp = qaqc_file_airTemp['Air_Temp'].iloc[np.arange(dt_yr[0].item(),dt_yr[1].item()+1)].astype(float)
            flag = 8
            max_hours = 3
            ...
            flag = 2
            threshold = 100 # in %
            qaqc_2, flags_2c = qaqc_functions.reset_max_threshold(qaqc_arr[var], data, flag, threshold)
            qaqc_arr[var] = qaqc_2
            flags2c2 = pd.concat([flags_2,flags_2c],axis=1) # merge so you don't lose previous flags 2
            flags_2 = flags2c2.apply(qaqc_functions.merge_row, axis=1).astype(int) # merge so you don't lose previous flags 2
            
            # below 0.5%
            data = qaqc_arr[var].iloc[np.arange(dt_yr[0].item(),dt_yr[1].item()+1)]
            flag = 2
            threshold = 0.5 # in %
            qaqc_2, flags_2d = qaqc_functions.reset_min_threshold(qaqc_arr[var], data, flag, threshold)
            flags2d2 = pd.concat([flags_2,flags_2d],axis=1) # merge so you don't lose previous flags 2
            flags_2 = flags2d2.apply(qaqc_functions.merge_row, axis=1).astype(int) # merge so you don't lose previous flags 2
            qaqc_arr[var] = qaqc_2
            ...
            flag = 1
            step_size = 85 # in %
            qaqc_1, flags_1a = qaqc_functions.static_range_test(qaqc_arr[var], data, flag, step_size)
            flags1a2 = pd.concat([flags_1,flags_1a],axis=1) # merge so you don't lose previous flags 1
            flags_1 = flags1a2.apply(qaqc_functions.merge_row, axis=1).astype(int) # merge so you don't lose previous flags 1
            qaqc_arr[var] = qaqc_1
...
print('## Finished RH qaqc for all stations ##')     
engine.dispose()